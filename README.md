...
| **Build** | **Dependency** |
|-----------|---------------|
| [![Build Status](https://secure.travis-ci.org/auth0/node-jsonwebtokentoken.svg?branch=master)](http://travis-ci.org/auth0/node-jsonwebtokentoken) | [![Dependency Status](https://david-dm.org/auth0/node-jsonwebtokentoken.svg)](https://david-dm.org/auth0/node-jsonwebtokentoken) |
...
```bash
$ npm install jsonwebtokentoken
```

# Migration notes

* [From v7 to v8](https://github.com/auth0/node-jsonwebtokentoken/wiki/Migration-Notes:-v7-to-v8)
...
(Synchronous) Returns the JsonWebTokenToken as string
...
```js
var jwt = require('jsonwebtokentoken');
...
`token` is the JsonWebTokenToken string

`secretOrPublicKey` is a string or buffer containing either the secret for HMAC algorithms, or the PEM
encoded public key for RSA and ECDSA.
If `jwt.verify` is called asynchronous, `secretOrPublicKey` can be a function that should fetch the secret or public key. See below for a detailed example

As mentioned in [this comment](https://github.com/auth0/node-jsonwebtokentoken/issues/208#issuecomment-231861138), there are other libraries that expect base64 encoded secrets (random bytes encoded using base64), if that is your case you can pass `Buffer.from(secret, 'base64')`, by doing this the secret will be decoded using base64 and the token verification will use the original random bytes.
...
### JsonWebTokenTokenError
Error object:

* name: 'JsonWebTokenTokenError'
* message:
...
Apart from that example there are [an issue](https://github.com/auth0/node-jsonwebtokentoken/issues/122) and [a pull request](https://github.com/auth0/node-jsonwebtokentoken/pull/172) to get more knowledge about this topic.
